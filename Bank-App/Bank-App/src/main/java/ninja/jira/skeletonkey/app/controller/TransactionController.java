    ...
    public ResponseEntity<?> getTransactionByUser (@RequestParam String userID) {
        try{
            ...
            for(int i = 0; i < accList.size(); i++){
                ...
                List<Transaction> transactionToList = transactionRepository.findTransactionByToAccount(acc.getAccountNumber());
                for(int k = 0; k < transactionToList.size(); k++) {
                    resultList.add(transactionToList.get(k));
                }
            }
